...
get_durations <- function(df, min_month) {
  ...
  winter_run_end <- as.Date(ifelse(df$rule2[min_row]==-1,df[csrl[run_no]-1,] %>% pull(x), NA))
  ...
  winter_sc_end <- as.Date(ifelse(df$belowlim[min_row]==-1,df[csrl_l[run_no_l]-1,] %>% pull(x), NA))
